...
class BarChartWidget(QWidget):
    ...
    def paintEvent(self, e):
        ...
        rect = QRectF(outer.left()+30, outer.top(), outer.width()-legend_w-30, outer.height()-36)
    ...
    def _on_intranet_ready(self, data: dict):
        try:
            ...
            for r in rows:
                ...
                enriched.append(rr)
            try:
                mapped_cnt = sum(1 for rr in enriched if rr.get('source_mapped'))
                feeder_cnt = sum(1 for r0 in (all_rows or []) if 'podajnik drutu' in str(r0.get('maszyna_opis','')).lower())
                missing = [rr.get('serial_no','') for rr in enriched if not rr.get('source_mapped')][:5]
                self._log(f"[Intranet] Mapped sources: {mapped_cnt}/{len(enriched)} | feeders_in_rows_all={feeder_cnt} | missing_sample={missing}")
            except Exception:
                pass
            self.intranet_rows = sorted(enriched, key=lambda r: r['data'])
